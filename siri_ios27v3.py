...
import os
import math
from PyQt6.QtCore import Qt, QTimer, QProcess, QPropertyAnimation, QPoint, QEasingCurve, QSettings
from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout,
                             QTextEdit, QLineEdit, QPushButton, QLabel, QFrame, QStackedWidget, QFileDialog)
...
class LockScreen(QWidget):
    def __init__(self):
        ...
        self.admin_btn.setFixedSize(35, 35)
    ...
    def admin(self):
      ...
      file_jpg = os.path.join(os.path.dirname(os.path.abspath(__file__)),
      "apple.jpg")
      ...
      password, ok = QInputDialog.getText(
        self,
        "Developer Mode",
        "Insert developer password:"
      )

      if not ok:
         return

      if password == "21092008":
         file_jpg = os.path.join(
        ...
        if say and reply:
            ...
            try:
                ...
                self.parent_widget.chat_area.append(
                    f"<span style=' background-color: rgb(15, 35, 70); border-radius: 5px; color: rgb(255, 255, 255); font-weight: bold;'>Siri:</span> I learned the new istruction! When you will say '{say}', I know what to say."
                )
            except Exception as e:
                self.parent_widget.chat_area.append(f"<span style='background-color: rgb(15, 35, 70); border-radius: 5px; color: rgb(255, 59, 48);'>Memory error: {str(e)}</span>")
...
class SiriIOS27(QWidget):
    ...
    def initUI(self):
        ...
        self.settings = QSettings("Siri_For_Raspberry", ".Siri_theme")

        self.setStyleSheet("""
            QWidget {
                background-color: rgba(120, 150, 180, 0.85);
                border-radius: 30px;
        ...
        self.chat_area.append("<span style='background-color: rgb(35, 45, 65); color: rgb(255, 255, 255); border-radius: 5px; font-weight: bold;'>Siri: I'm starting the app, please wait a moment.</span>")
        QTimer.singleShot(5000, lambda: [
            self.chat_area.append("<span style='background-color: rgb(35, 45, 65); color: rgb(255, 255, 255); border-radius: 5px; font-weight: bold;'>Siri: Hi, now I'm ready. Ask me something.</span>")
            ])
        self.chat_page_layout.addWidget(self.chat_area)

        self.clear_chat_btn = QPushButton("🗑", self)
        ...
        sfondo = self.settings.value("chat_background", "")

        if sfondo:
            self.chat_area.viewport().setStyleSheet(f"""
...
# --- GRAFICA PAGINA IMPOSTAZIONI ---
        ...
        #Salvataggio tema
        tema_salvato = self.settings.value("tema_attivo", "Liquid")

        if tema_salvato == "Dark":
            self.chat_page.setStyleSheet("background-color: rgba(20, 35, 60, 0.80); border-radius: 22px;")
            self.settings_page.setStyleSheet("background-color: rgba(20, 35, 60, 0.80); border-radius: 22px;")

        elif tema_salvato == "Light":
            self.chat_page.setStyleSheet("background-color: rgba(100, 130, 160, 0.5); border-radius: 22px;")
            self.settings_page.setStyleSheet("background-color: rgba(100, 130, 160, 0.5); border-radius: 22px;")

        elif tema_salvato == "Liquid":
        ...
        self.dark_theme_btn.clicked.connect(lambda: [
            self.chat_page.setStyleSheet("background-color: rgba(20, 35, 60, 0.80); border-radius: 22px;"),
            self.settings_page.setStyleSheet("background-color: rgba(20, 35, 60, 0.80); border-radius: 22px;"),
            self.settings.setValue("tema_attivo", "Dark")
        ])

        self.light_theme_btn.clicked.connect(lambda: [
            self.chat_page.setStyleSheet("background-color: rgba(100, 130, 160, 0.5); border-radius: 22px;"),
            self.settings_page.setStyleSheet("background-color: rgba(100, 130, 160, 0.5); border-radius: 22px;"),
            self.settings.setValue("tema_attivo", "Light")
        ])

        self.glass_theme_btn.clicked.connect(lambda: [
            self.chat_page.setStyleSheet(""),
            self.settings_page.setStyleSheet(""),
            self.settings.setValue("tema_attivo", "Liquid")
        ])

        self.settings_page_layout.addStretch() # Spinge tutto verso l'alto

        self.reset_theme_label = QLabel("<u>​‌RESET default chat theme</u>", self)
        self.reset_theme_label.setStyleSheet("""
          color: #00f2fe;
          background: transparent;
          font-size: 12;
          border-radius: 12px;
        ...
        self.reset_theme_label.setCursor(Qt.CursorShape.PointingHandCursor)

        self.settings_page_layout.addWidget(self.reset_theme_label)
        self.reset_theme_label.mousePressEvent = self.reset_chat_theme


        self.background_btn = QPushButton(" Change chat background")
        self.settings_page_layout.addWidget(self.background_btn)
        ...
        self.back_btn.clicked.connect(self.open_chat)
    ...
    def trigger_proposal(self):
        self.chat_area.append("<hr><span style='color: #ffffff; font-weight: bold;'</span>")
...
Siri() {
    ...
    local proposte=(
        "Do you want to know how to make your computer faster?"
        "Do you want to check how much RAM your computer is using?"
        "Do you want to check your processor temperature?"
        "Do you want to customize Siri's appearance?"
        "Do you want to teach me a new command?"
        "Do you want to know usefull Linux commands?"
        "Do you want to clean unnecessary files?"
        "Do you want to see your system information?"
        "Do you want to improve battery life?"
        "Do you want to know waht Siri can do?"
    )
    ...
    while true; do
        ...
        if [ -z "$frase_utente" ]; then
            ...
            if [ "$conferma_prop" = "yes" ] || [ "$conferma_prop" = "yap" ]; then
                echo ""
                case "$index" in
                    0) echo "Siri: To make your computer faster, you can close unused programs, clean temporary files and reduce startup applications.";;
                    1) echo "Siri: You can check your RAM usage by typing 'free -h' in the terminal. Keeping some memory free helps the system stay fast.";;
                    2) echo "Siri: You can check the CPU temperature with the command ' vcgencmd measure_temp'.";;
                    3) echo "Siri: You can customize me from Settings. You can change themes, colors and chat background.";;
                    4) echo "Siri: Press the teach button to tach me new answers or commands that I will remember.(P.S. If you want to delete something that you teach me say delete.)";;
                    5) echo "Siri: Some useful Linux commands are: ls, cd, mkdir, free, top and sudo.";;
                    6) echo "Siri: Removing unused files and old packages can help save storage space.";;
                    7) echo "Siri: Ask me 'operative system' and I will show you information about your computer.";;
                    8) echo "Siri: To save battery, reduce screen breightness close unused apps and avoid heavy background process.";;
                    9) echo "Siri: I can answer questions, open programms, execute commands, learn new phrases and help you manage your computer.";;
                    *) echo "";;
                esac
            else
        ...
        if [[ "$pulita" =~ [0-9]+[[:space:]]*[-+*/][[:space:]]*[0-9] ]]; then
          operazione=$(echo "$pulita" | grep -oE '[0-9]+[-+*/][0-9]+')
          risultato=$(echo "$operazione" | bc -l 2>/dev/null)

          if [ -n "$risultato" ]; then
            echo " Siri: The result is $risultato"
            echo ""
            continue
          else
            echo "Siri: Sorry, I couldn't calculate that ERROR: $operazione."
          fi
        fi
        ...
        case "$pulita" in
            *"good afternoon"*|*"good morning"*|*"good evening"*|*"hey"*|*"hey siri"*|*"hi"*|*"hello"*)
                echo "Siri: Hi! How I can help you?"
                ;;
            *"how is it going"*|*"how it's going"*|*"how's it going"*|*"how are you"*)
                ...
                case "$response" in
                  *"yes"*|*"yeah"*|*"yap"*|*"yep"*|*"sure"*|*"ok"*)
                    echo "Siri: I'm happy that you're well 😊"
                    ;;
                  *)
                    echo "Siri: I hope things get better. I'm here If you need help."
                    ;;
                esac
                ;;
            ...
            *"no"*)
            ...
            "RESETSiri"|*"Siri do a RESET of your memory"*)
                ...
                if [ "$conferma1" = "yes" ] || [ "$conferma1" = "yap" ]; then
                    ...
                    echo "Siri > Do you want to do the RESET? (yes/no): "
                    read -p " " conferma2
           ...
           *"Siri"*)
               echo "I'm ready."
               ;;
           *"what can you do"*|*"What can I do with you"*|*"what you can do"*|*"what I can do with you"*)
               echo "-Open applications"
               echo "-Search files"
               echo "-Learn new commands"
               echo "-Manage your system"
               echo "-Change my appearance"
               echo "-Give computer tips"
               echo "-Check the weather(If you have internet)"
               ;;
           *"calculate"*|*"what is"*|*"how much is"*)
               operazione=$(echo "$pulita" | sed -E 's/(how much is|calculate|what is)//g' | xargs)
               ...
               if [ -n "$risultato" ]; then
                 ...
                 echo "Siri: Sorry, I couldn't calculate that."
               fi
               ;;
           *"help me"*)
               echo "Siri: What happen?"
               ;;
           *"thank you"*|*"thanks"*)
               echo "Siri: You're welcome! I'm always ready to help."
               ;;
           *"Good job"*)
               echo "Siri: Thank you! I'm just doing my job 😊"
               ;;
            *)
                ...
                echo "Siri: I don't know this yet, "
                echo ""
                echo "But you can press Teach and tell me what I should answer next time."
...
"""
    ...
    def process_command(self):
        ...
        self.chat_area.append(f"<hr><span style='background-color: rgb(0, 122, 255); color: rgb(255, 255, 255); font-weight: bold; border-radius: 5px; padding: 5px;'>You: {text}</span>")
    ...
    def read_bash_output(self):
        ...
        if self.attiva_output and data.strip():
            ...
            if output_filtrato:
                ...
                self.chat_area.append(f"<span style='background-color: rgb(55, 55, 55); color: rgb(255, 255, 255); font-weight: bold; border-radius: 5px; padding: 5px;'>{testo_finale}</span>")
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'clear_chat_btn'):
            self.clear_chat_btn.move(400, 490)
            self.clear_chat_btn.raise_()

        if hasattr(self, 'admin_btn'):
            self.admin_btn.move(43, 490)
            self.admin_btn.raise_()

    def clear_chat(self):
      self.chat_area.clear()

      self.chat_area.append("<span style='background-color: rgb(35, 45, 65); color: rgb(255, 255, 255); font-weigth:bold;'>Siri: I'm deleting the chat</span>")
      QTimer.singleShot(1000, lambda: [
       self.chat_area.clear(),
       QTimer.singleShot(500, lambda: [
       self.process.write("exit\n".encode()),
       self.process.write("Siri\n".encode()),
       self.chat_area.append("<span style='background-color: rgb(35, 45, 65); color: rgb(255, 255, 255); font-weight: bold;'>Siri:</span> Hi, now I'm ready. Ask me something.")
       ])
      ])
    ...
    def change_background(self):
      ...
      if file:
        self.chat_area.viewport().setStyleSheet(f"""
            ...
            background-position: center;
        """)
        self.settings.setValue("chat_background", file)

    def open_settings(self):
      self.pages.setCurrentIndex(1)
      self.clear_chat_btn.hide()
      self.admin_btn.hide()

    def open_chat(self):
      self.pages.setCurrentIndex(0)
      self.clear_chat_btn.show()
      self.admin_btn.show()

    def reset_chat_theme(self, event):
      self.chat_area.viewport().setStyleSheet("")
      self.settings.remove("chat_background")
